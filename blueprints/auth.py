...
def register():
    ...
    if request.method == 'POST':
        ...
        if password != current_app.config.get('FAMILY_PASSWORD'):
            flash(
                'Incorrect Family Code. Please ask the family admin.',
                'danger')
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            login_user(user)
            current_app.logger.info(
                f'User logged in: {email} (user_id={user.id})')
...
def forgot_email():
    ...
    if request.method == 'POST':
        ...
        if len(users) == 1:
            ...
            user = users[0]
            current_app.logger.info(
                f'Email recovery successful for: {user.name}')
            ...
            msg = (f'We found {len(users)} accounts with similar '
                   f'names. Please contact support.')
            flash(msg, 'warning')
            return render_template('forgot_email.html', name=name)
        else:
            # No match found
            current_app.logger.warning(
                f'Email recovery failed for name: {name}')
            flash(
                'We could not find an account with that name. '
                'Please check your spelling or sign up.',
                'danger')
            return render_template('forgot_email.html', name=name)
