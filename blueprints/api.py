...
def extraction_health():
    ...
    try:
        ...
        stats = db.session.query(
            ...
            func.sum(
                case((PriceExtractionLog.success, 1), else_=0)
            ).label('success')
        ).filter(
...
def get_item_price_history(item_id):
    ...
    # Verify item exists (and optionally permissions, though read-only is low
    # risk)
    ...
    try:
        ...
        current_app.logger.error(
            f"Price history fetch failed for {item_id}: {e}")
        return jsonify({'error': str(e)}), 500
