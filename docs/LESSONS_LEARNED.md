...
### 7. Browser Fetching Is Not Enough for Etsy / Home Depot (anti-bot shells)
- **Issue (2026-08-13):** Etsy (5 items) and Home Depot (3) returned 0% price-history. Both serve a ~1.5–2.5KB JS bootloader shell to *any* automated browser and yield no price markup. Verified exhaustively: headless chromium, headed chromium, and real installed Chrome (`channel="chrome"`) all get the shell; Etsy also hangs/226s the playwright MCP browser tab. This is Etsy's Sistine-style gate and HD's JS-only product shell — they require residential proxies + CAPTCHA-solving (commercial anti-bot infra) to scrape reliably.
- **Solution:** Don't re-explore these without commercial infra. The wins that DO work: Target-style retailers 200 aiohttp but render the price only in a browser — fix = treat "HTTP 200 with no parseable price" as a browser-rescue candidate and never cache no-price responses (Target went 0% → 100%: $31.49/$55.00 extracted). General rule: before concluding a site is hard, check whether aiohttp's 200 has the price in the DOM; if not, try the browser path; only then call it a shell.
