...
> **STATUS: Complete and superseded.** Every step below is implemented and now
> committed. The API is the backend for the `boundary-ios` arrow segment (LLDs in
> `docs/intent/boundary-ios/`, per-segment arrow docs under
> `docs/arrows/boundary-ios/`). This plan is retained as the original build record;
> new API work goes through the arrow, not this file.
...
- [x] **Step 3: Add the request_loader**
...
- [x] **Step 4: Run test to verify it passes, plus regression check**
...
- [x] **Step 3: Implement the serializers**
...
- [x] **Step 5: Refactor the web routes to use the service**
...
- [x] **Step 6: Regression check — full unit suite**
...
- [x] **Step 4: Register the blueprint, CSRF-exempt**
...
- [x] **Step 5: Run test to verify it passes, plus regression**
...
- [x] **Step 1: Move shared validation into `services/form_validators.py`**
...
- [x] **Step 1: Add dependencies and config**
...
- [x] **Step 4: Implement push service, notification service, Celery task, device routes**
...
- [x] **Step 5: Run tests to verify they pass, plus full regression**
...
- [x] **Step 1: Run the complete suite (unit + browser) with coverage**
...
- [x] **Step 3: Update `CLAUDE.md` and `.env.example`**
...
Deployment now happens through the server runbooks, not this plan: production is
Heroku (run `heroku run flask db upgrade` manually when migrations exist — release
phase is disabled, per `docs/LESSONS_LEARNED.md`), and the sandbox environment
lives on the hp-server SSH box (`docs/DEPLOY_HOME_SERVER.md`). Plan 2 (the SwiftUI
iOS app) was completed — see the `boundary-ios` arrow segment.
