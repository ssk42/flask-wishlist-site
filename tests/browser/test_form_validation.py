...
def test_edit_item_status_picklist_options(page, live_server):
    ...
    # Navigate to edit - owner sees priority (not status - that's for non-owners)
    page.goto(f"{live_server}/items")
    page.wait_for_load_state('networkidle')
    
    # Click Edit link
    edit_link = page.locator(f'.glass-card:has-text("{item_desc}") a:has-text("Edit")').first
...
def test_items_filter_status_picklist_options(page, live_server):
    ...
    page.goto(f"{live_server}/submit_item")
    page.fill('input[name="description"]', f'Filter Test {uuid.uuid4().hex[:8]}')
    page.click('button[type="submit"]')
    page.wait_for_load_state('networkidle')
...
def test_items_filter_user_picklist_populated(page, live_server):
    ...
    user_email = register_and_login(page, live_server, name="Filter Test User")
    page.goto(f"{live_server}/items")
    ...
    assert 'Filter Test User' in options_text or any('Filter Test' in opt for opt in options_text)
