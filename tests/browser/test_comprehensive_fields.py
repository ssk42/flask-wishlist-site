...
def test_navbar_shows_all_links_when_logged_in(page, live_server):
    ...
    # Check for main navigation links - use .first since sidebar/mobile may duplicate
    expect(page.locator('a:has-text("Home")').first).to_be_visible()
    expect(page.locator('a:has-text("All Gifts"), a:has-text("Items")').first).to_be_visible()
...
def test_success_flash_message_displays(page, live_server):
    ...
    page.fill('input[name="description"]', f"Flash Test {uuid.uuid4().hex[:8]}")
    page.click('button[type="submit"]')
...
def test_warning_flash_message_displays(page, live_server):
    ...
    user_email = register_and_login(page, live_server)
    ...
    claim_btn = item_card.locator('button:has-text("Claim"), a:has-text("Claim")')
...
def test_quick_view_modal_opens(page, live_server):
    ...
    page.goto(f"{live_server}/")
    quick_view_btn = page.locator(f'button:has-text("Quick View")').first
